...
function generateAllPeriods() {
    ...
    allDebts.forEach(debt => {
        ...
        let currentDate = new Date(debt.dueDate); // Берем дату из поля "Первый срок"
        if (isNaN(currentDate.getTime())) {
        // Если дата некорректна, используем дату создания
...
function renderDebts() {
    ...
    // СОРТИРОВКА: неоплаченные по дате (просроченные первыми), оплаченные в конце
    if (currentFilter !== 'archive') {
        const today = new Date();
        today.setHours(0, 0, 0, 0);

        filtered.sort((a, b) => {
            // Функция получения следующей даты платежа
            const getDueDate = (debt) => {
                if ((debt.paidAmount || 0) >= debt.amount && !(debt.repeatEnabled && debt.repeatType !== 'none')) {
                    return null;
                }
                if (debt.repeatEnabled && debt.repeatType !== 'none') {
                    const periods = (debt.periods || []).sort((a, b) => new Date(a.dueDate) - new Date(b.dueDate));
                    const next = periods.find(p => (p.paidAmount || 0) < p.amount);
                    return next ? new Date(next.dueDate + 'T00:00:00') : null;
                } else {
                    if (debt.dueDate) return new Date(debt.dueDate + 'T00:00:00');
                    return null;
                }
            };
            ...
            // Оба неоплаченные — сортируем по дате (просроченные будут раньше)
            ...
            // Неоплаченные первыми
            if (aIsUnpaid && !bIsUnpaid) return -1;
            if (!aIsUnpaid && bIsUnpaid) return 1;
            // Оба оплаченные — оставляем как есть (или можно сортировать по дате оплаты)
            return 0;
        });
    } else {
        filtered.sort((a, b) => {
